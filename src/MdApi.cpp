...
static PyObject *CTP_THOST_FTDC_MD_API_CreateFtdcMdApi(PyTypeObject *type, PyObject *args)
{
	...
	int bIsUsingUdp = false, bIsMulticast = false, bIsProductionMode = true;
#if PY_MAJOR_VERSION >= 3
	if (!PyArg_ParseTuple(args, "|yppp", &pszFlowPath, &bIsUsingUdp, &bIsMulticast, &bIsProductionMode))
#else
    if (!PyArg_ParseTuple(args, "|siii", &pszFlowPath, &bIsUsingUdp, &bIsMulticast, &bIsProductionMode))
#endif
	{
	...
	/* 创建ctpapi实例 */
	self->api = CThostFtdcMdApi::CreateFtdcMdApi(pszFlowPath==NULL? "":pszFlowPath, bIsUsingUdp, bIsMulticast, bIsProductionMode);
