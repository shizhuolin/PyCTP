...
int PyCTP_All_Type_init(PyObject *module) {
	...
	///App客户端认证码
	if( PyCThostFtdcAppAuthenticationCodeFieldType_init(module) < 0 ) return -1;
	///客户中心权限豁免
	if( PyCThostFtdcUserDRIBypassFieldType_init(module) < 0 ) return -1;
	///申请短信验证码请求
	if( PyCThostFtdcReqGenSMSCodeFieldType_init(module) < 0 ) return -1;
	///申请短信验证码响应
	if( PyCThostFtdcRspGenSMSCodeFieldType_init(module) < 0 ) return -1;
	///短信验证信息通知
	if( PyCThostFtdcSMSVerifyInfoFromSecFieldType_init(module) < 0 ) return -1;
	///登录验证设置
	if( PyCThostFtdcSMSVerifyConfigFieldType_init(module) < 0 ) return -1;
	///短信验证信息通知
	if( PyCThostFtdcSMSVerifyInfoFieldType_init(module) < 0 ) return -1;
	///套利确认输入基本信息
	if( PyCThostFtdcInputSpdApplyFieldType_init(module) < 0 ) return -1;
	///套保确认输入基本信息
	if( PyCThostFtdcInputHedgeCfmFieldType_init(module) < 0 ) return -1;
	///套利申请回报
	if( PyCThostFtdcSpdApplyFieldType_init(module) < 0 ) return -1;
	///套保申请回报
	if( PyCThostFtdcHedgeCfmFieldType_init(module) < 0 ) return -1;
	///套利套保申请查询
	if( PyCThostFtdcQrySpdApplyFieldType_init(module) < 0 ) return -1;
	///套利套保申请查询
	if( PyCThostFtdcQryHedgeCfmFieldType_init(module) < 0 ) return -1;
	///套利申请撤销
	if( PyCThostFtdcInputSpdApplyActionFieldType_init(module) < 0 ) return -1;
	///套保申请撤销
	if( PyCThostFtdcInputHedgeCfmActionFieldType_init(module) < 0 ) return -1;
	///套利申请撤销回报
	if( PyCThostFtdcSpdApplyActionFieldType_init(module) < 0 ) return -1;
	///套保申请撤销回报
	if( PyCThostFtdcHedgeCfmActionFieldType_init(module) < 0 ) return -1;
	///前置信息
