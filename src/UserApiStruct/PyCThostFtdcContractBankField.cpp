...
static int PyCThostFtdcContractBankField_init(PyCThostFtdcContractBankField *self, PyObject *args, PyObject *kwds) {
    ...
    ///上报csrc的银行代码
    // TThostFtdcBankIDType char[4]
    const char *ContractBankField_csrcBankID = NULL;
    Py_ssize_t ContractBankField_csrcBankID_len = 0;
    ...
    if (!PyArg_ParseTupleAndKeywords(args, kwds, "|s#s#s#s#s#", (char **)kwlist
        ...
        , &ContractBankField_csrcBankID, &ContractBankField_csrcBankID_len 
    ...
    ///上报csrc的银行代码
    // TThostFtdcBankIDType char[4]
    if( ContractBankField_csrcBankID != NULL ) {
        if(ContractBankField_csrcBankID_len >= (Py_ssize_t)sizeof(self->data.csrcBankID)) {
            PyErr_Format(PyExc_ValueError, "csrcBankID too long: length=%zd (max allowed is %zd)", ContractBankField_csrcBankID_len, (Py_ssize_t)sizeof(self->data.csrcBankID));
            return -1;
        }
        // memset(self->data.csrcBankID, 0, sizeof(self->data.csrcBankID));
        // memcpy(self->data.csrcBankID, ContractBankField_csrcBankID, ContractBankField_csrcBankID_len);        
        strncpy(self->data.csrcBankID, ContractBankField_csrcBankID, sizeof(self->data.csrcBankID) );
        ContractBankField_csrcBankID = NULL;
    }
...
static PyObject *PyCThostFtdcContractBankField_repr(PyCThostFtdcContractBankField *self) {
    ...
    PyObject *obj = Py_BuildValue("{s:s,s:s,s:s,s:s,s:s}"
        ...
        ,"csrcBankID", self->data.csrcBankID//, (Py_ssize_t)sizeof(self->data.csrcBankID) 
...
///上报csrc的银行代码
// TThostFtdcBankIDType char[4]
static PyObject *PyCThostFtdcContractBankField_get_csrcBankID(PyCThostFtdcContractBankField *self, void *closure) {
    //return PyBytes_FromStringAndSize(self->data.csrcBankID, (Py_ssize_t)sizeof(self->data.csrcBankID));
    return PyBytes_FromString(self->data.csrcBankID);
}

///上报csrc的银行代码
// TThostFtdcBankIDType char[4]
static int PyCThostFtdcContractBankField_set_csrcBankID(PyCThostFtdcContractBankField *self, PyObject* val, void *closure) {
    if (!PyBytes_Check(val)) {
    ...
    const char *buf = PyBytes_AsString(val);
    Py_ssize_t len = PyBytes_Size(val);
    if (len >= (Py_ssize_t)sizeof(self->data.csrcBankID)) {
        PyErr_SetString(PyExc_ValueError, "csrcBankID must be less than 4 bytes");
        return -1;
    }
    // memset(self->data.csrcBankID, 0, sizeof(self->data.csrcBankID));
    // memcpy(self->data.csrcBankID, buf, len);
    strncpy(self->data.csrcBankID, buf, sizeof(self->data.csrcBankID));
    return 0;
}
...
static PyGetSetDef PyCThostFtdcContractBankField_getset[] = {
    ...
    ///上报csrc的银行代码 
    {(char *)"csrcBankID", (getter)PyCThostFtdcContractBankField_get_csrcBankID, (setter)PyCThostFtdcContractBankField_set_csrcBankID, (char *)"csrcBankID", NULL},
