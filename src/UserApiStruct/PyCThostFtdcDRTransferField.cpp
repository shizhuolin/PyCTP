...
static int PyCThostFtdcDRTransferField_init(PyCThostFtdcDRTransferField *self, PyObject *args, PyObject *kwds) {
	...
	const char *pDRTransferField_OrigBrokerID = NULL;
	Py_ssize_t pDRTransferField_OrigBrokerID_len = 0;

	//TThostFtdcBrokerIDType char[11]
	const char *pDRTransferField_DestBrokerID = NULL;
	Py_ssize_t pDRTransferField_DestBrokerID_len = 0;
    ...
    if (!PyArg_ParseTupleAndKeywords(args, kwds, "|iis#s#", (char **)kwlist
		...
		, &pDRTransferField_OrigBrokerID, &pDRTransferField_OrigBrokerID_len
		, &pDRTransferField_DestBrokerID, &pDRTransferField_DestBrokerID_len
	...
	//TThostFtdcDRIdentityIDType int
	self->data.OrigDRIdentityID = pDRTransferField_OrigDRIdentityID;

	//TThostFtdcDRIdentityIDType int
	self->data.DestDRIdentityID = pDRTransferField_DestDRIdentityID;

	//TThostFtdcBrokerIDType char[11]
	if(pDRTransferField_OrigBrokerID != NULL) {
		if(pDRTransferField_OrigBrokerID_len > (Py_ssize_t)sizeof(self->data.OrigBrokerID)) {
			PyErr_Format(PyExc_ValueError, "OrigBrokerID too long: length=%zd (max allowed is %zd)", pDRTransferField_OrigBrokerID_len, (Py_ssize_t)sizeof(self->data.OrigBrokerID));
			return -1;
		}
		strncpy(self->data.OrigBrokerID, pDRTransferField_OrigBrokerID, sizeof(self->data.OrigBrokerID) );
		pDRTransferField_OrigBrokerID = NULL;
	}

	//TThostFtdcBrokerIDType char[11]
	if(pDRTransferField_DestBrokerID != NULL) {
		if(pDRTransferField_DestBrokerID_len > (Py_ssize_t)sizeof(self->data.DestBrokerID)) {
			PyErr_Format(PyExc_ValueError, "DestBrokerID too long: length=%zd (max allowed is %zd)", pDRTransferField_DestBrokerID_len, (Py_ssize_t)sizeof(self->data.DestBrokerID));
			return -1;
		}
		strncpy(self->data.DestBrokerID, pDRTransferField_DestBrokerID, sizeof(self->data.DestBrokerID) );
		pDRTransferField_DestBrokerID = NULL;
	}
...
static PyObject *PyCThostFtdcDRTransferField_repr(PyCThostFtdcDRTransferField *self) {
    ...
    PyObject *obj = Py_BuildValue("{s:i,s:i,s:s,s:s}"
		...
		, "OrigBrokerID", self->data.OrigBrokerID 
		, "DestBrokerID", self->data.DestBrokerID 
