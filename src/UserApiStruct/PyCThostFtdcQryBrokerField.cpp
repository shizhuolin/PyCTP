...
static int PyCThostFtdcQryBrokerField_init(PyCThostFtdcQryBrokerField *self, PyObject *args, PyObject *kwds) {
	...
	const char *pQryBrokerField_BrokerID = NULL;
	Py_ssize_t pQryBrokerField_BrokerID_len = 0;
    ...
    if (!PyArg_ParseTupleAndKeywords(args, kwds, "|s#", (char **)kwlist
		...
		, &pQryBrokerField_BrokerID, &pQryBrokerField_BrokerID_len
	...
	//TThostFtdcBrokerIDType char[11]
	if(pQryBrokerField_BrokerID != NULL) {
		if(pQryBrokerField_BrokerID_len > (Py_ssize_t)sizeof(self->data.BrokerID)) {
			PyErr_Format(PyExc_ValueError, "BrokerID too long: length=%zd (max allowed is %zd)", pQryBrokerField_BrokerID_len, (Py_ssize_t)sizeof(self->data.BrokerID));
			return -1;
		}
		strncpy(self->data.BrokerID, pQryBrokerField_BrokerID, sizeof(self->data.BrokerID) );
		pQryBrokerField_BrokerID = NULL;
	}
