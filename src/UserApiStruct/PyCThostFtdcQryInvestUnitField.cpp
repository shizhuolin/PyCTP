...
static int PyCThostFtdcQryInvestUnitField_init(PyCThostFtdcQryInvestUnitField *self, PyObject *args, PyObject *kwds) {
	...
	const char *pQryInvestUnitField_BrokerID = NULL;
	Py_ssize_t pQryInvestUnitField_BrokerID_len = 0;

	//TThostFtdcInvestorIDType char[13]
	const char *pQryInvestUnitField_InvestorID = NULL;
	Py_ssize_t pQryInvestUnitField_InvestorID_len = 0;

	//TThostFtdcInvestUnitIDType char[17]
	const char *pQryInvestUnitField_InvestUnitID = NULL;
	Py_ssize_t pQryInvestUnitField_InvestUnitID_len = 0;
    ...
    if (!PyArg_ParseTupleAndKeywords(args, kwds, "|s#s#s#", (char **)kwlist
		...
		, &pQryInvestUnitField_BrokerID, &pQryInvestUnitField_BrokerID_len
		, &pQryInvestUnitField_InvestorID, &pQryInvestUnitField_InvestorID_len
		, &pQryInvestUnitField_InvestUnitID, &pQryInvestUnitField_InvestUnitID_len
	...
	//TThostFtdcBrokerIDType char[11]
	if(pQryInvestUnitField_BrokerID != NULL) {
		if(pQryInvestUnitField_BrokerID_len > (Py_ssize_t)sizeof(self->data.BrokerID)) {
			PyErr_Format(PyExc_ValueError, "BrokerID too long: length=%zd (max allowed is %zd)", pQryInvestUnitField_BrokerID_len, (Py_ssize_t)sizeof(self->data.BrokerID));
			return -1;
		}
		strncpy(self->data.BrokerID, pQryInvestUnitField_BrokerID, sizeof(self->data.BrokerID) );
		pQryInvestUnitField_BrokerID = NULL;
	}

	//TThostFtdcInvestorIDType char[13]
	if(pQryInvestUnitField_InvestorID != NULL) {
		if(pQryInvestUnitField_InvestorID_len > (Py_ssize_t)sizeof(self->data.InvestorID)) {
			PyErr_Format(PyExc_ValueError, "InvestorID too long: length=%zd (max allowed is %zd)", pQryInvestUnitField_InvestorID_len, (Py_ssize_t)sizeof(self->data.InvestorID));
			return -1;
		}
		strncpy(self->data.InvestorID, pQryInvestUnitField_InvestorID, sizeof(self->data.InvestorID) );
		pQryInvestUnitField_InvestorID = NULL;
	}

	//TThostFtdcInvestUnitIDType char[17]
	if(pQryInvestUnitField_InvestUnitID != NULL) {
		if(pQryInvestUnitField_InvestUnitID_len > (Py_ssize_t)sizeof(self->data.InvestUnitID)) {
			PyErr_Format(PyExc_ValueError, "InvestUnitID too long: length=%zd (max allowed is %zd)", pQryInvestUnitField_InvestUnitID_len, (Py_ssize_t)sizeof(self->data.InvestUnitID));
			return -1;
		}
		strncpy(self->data.InvestUnitID, pQryInvestUnitField_InvestUnitID, sizeof(self->data.InvestUnitID) );
		pQryInvestUnitField_InvestUnitID = NULL;
	}
