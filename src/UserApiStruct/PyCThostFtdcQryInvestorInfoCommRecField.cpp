...
static int PyCThostFtdcQryInvestorInfoCommRecField_init(PyCThostFtdcQryInvestorInfoCommRecField *self, PyObject *args, PyObject *kwds) {
	...
	const char *pQryInvestorInfoCommRecField_InvestorID = NULL;
	Py_ssize_t pQryInvestorInfoCommRecField_InvestorID_len = 0;

	//TThostFtdcInstrumentIDType char[81]
	const char *pQryInvestorInfoCommRecField_InstrumentID = NULL;
	Py_ssize_t pQryInvestorInfoCommRecField_InstrumentID_len = 0;

	//TThostFtdcBrokerIDType char[11]
	const char *pQryInvestorInfoCommRecField_BrokerID = NULL;
	Py_ssize_t pQryInvestorInfoCommRecField_BrokerID_len = 0;
    ...
    if (!PyArg_ParseTupleAndKeywords(args, kwds, "|s#s#s#", (char **)kwlist
		...
		, &pQryInvestorInfoCommRecField_InvestorID, &pQryInvestorInfoCommRecField_InvestorID_len
		, &pQryInvestorInfoCommRecField_InstrumentID, &pQryInvestorInfoCommRecField_InstrumentID_len
		, &pQryInvestorInfoCommRecField_BrokerID, &pQryInvestorInfoCommRecField_BrokerID_len
	...
	//TThostFtdcInvestorIDType char[13]
	if(pQryInvestorInfoCommRecField_InvestorID != NULL) {
		if(pQryInvestorInfoCommRecField_InvestorID_len > (Py_ssize_t)sizeof(self->data.InvestorID)) {
			PyErr_Format(PyExc_ValueError, "InvestorID too long: length=%zd (max allowed is %zd)", pQryInvestorInfoCommRecField_InvestorID_len, (Py_ssize_t)sizeof(self->data.InvestorID));
			return -1;
		}
		strncpy(self->data.InvestorID, pQryInvestorInfoCommRecField_InvestorID, sizeof(self->data.InvestorID) );
		pQryInvestorInfoCommRecField_InvestorID = NULL;
	}

	//TThostFtdcInstrumentIDType char[81]
	if(pQryInvestorInfoCommRecField_InstrumentID != NULL) {
		if(pQryInvestorInfoCommRecField_InstrumentID_len > (Py_ssize_t)sizeof(self->data.InstrumentID)) {
			PyErr_Format(PyExc_ValueError, "InstrumentID too long: length=%zd (max allowed is %zd)", pQryInvestorInfoCommRecField_InstrumentID_len, (Py_ssize_t)sizeof(self->data.InstrumentID));
			return -1;
		}
		strncpy(self->data.InstrumentID, pQryInvestorInfoCommRecField_InstrumentID, sizeof(self->data.InstrumentID) );
		pQryInvestorInfoCommRecField_InstrumentID = NULL;
	}

	//TThostFtdcBrokerIDType char[11]
	if(pQryInvestorInfoCommRecField_BrokerID != NULL) {
		if(pQryInvestorInfoCommRecField_BrokerID_len > (Py_ssize_t)sizeof(self->data.BrokerID)) {
			PyErr_Format(PyExc_ValueError, "BrokerID too long: length=%zd (max allowed is %zd)", pQryInvestorInfoCommRecField_BrokerID_len, (Py_ssize_t)sizeof(self->data.BrokerID));
			return -1;
		}
		strncpy(self->data.BrokerID, pQryInvestorInfoCommRecField_BrokerID, sizeof(self->data.BrokerID) );
		pQryInvestorInfoCommRecField_BrokerID = NULL;
	}
