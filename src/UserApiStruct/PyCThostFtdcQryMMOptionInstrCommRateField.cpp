...
static int PyCThostFtdcQryMMOptionInstrCommRateField_init(PyCThostFtdcQryMMOptionInstrCommRateField *self, PyObject *args, PyObject *kwds) {
	...
	const char *pQryMMOptionInstrCommRateField_BrokerID = NULL;
	Py_ssize_t pQryMMOptionInstrCommRateField_BrokerID_len = 0;

	//TThostFtdcInvestorIDType char[13]
	const char *pQryMMOptionInstrCommRateField_InvestorID = NULL;
	Py_ssize_t pQryMMOptionInstrCommRateField_InvestorID_len = 0;

	//TThostFtdcOldInstrumentIDType char[31]
	const char *pQryMMOptionInstrCommRateField_reserve1 = NULL;
	Py_ssize_t pQryMMOptionInstrCommRateField_reserve1_len = 0;

	//TThostFtdcInstrumentIDType char[81]
	const char *pQryMMOptionInstrCommRateField_InstrumentID = NULL;
	Py_ssize_t pQryMMOptionInstrCommRateField_InstrumentID_len = 0;
    ...
    if (!PyArg_ParseTupleAndKeywords(args, kwds, "|s#s#s#s#", (char **)kwlist
		...
		, &pQryMMOptionInstrCommRateField_BrokerID, &pQryMMOptionInstrCommRateField_BrokerID_len
		, &pQryMMOptionInstrCommRateField_InvestorID, &pQryMMOptionInstrCommRateField_InvestorID_len
		, &pQryMMOptionInstrCommRateField_reserve1, &pQryMMOptionInstrCommRateField_reserve1_len
		, &pQryMMOptionInstrCommRateField_InstrumentID, &pQryMMOptionInstrCommRateField_InstrumentID_len
	...
	//TThostFtdcBrokerIDType char[11]
	if(pQryMMOptionInstrCommRateField_BrokerID != NULL) {
		if(pQryMMOptionInstrCommRateField_BrokerID_len > (Py_ssize_t)sizeof(self->data.BrokerID)) {
			PyErr_Format(PyExc_ValueError, "BrokerID too long: length=%zd (max allowed is %zd)", pQryMMOptionInstrCommRateField_BrokerID_len, (Py_ssize_t)sizeof(self->data.BrokerID));
			return -1;
		}
		strncpy(self->data.BrokerID, pQryMMOptionInstrCommRateField_BrokerID, sizeof(self->data.BrokerID) );
		pQryMMOptionInstrCommRateField_BrokerID = NULL;
	}

	//TThostFtdcInvestorIDType char[13]
	if(pQryMMOptionInstrCommRateField_InvestorID != NULL) {
		if(pQryMMOptionInstrCommRateField_InvestorID_len > (Py_ssize_t)sizeof(self->data.InvestorID)) {
			PyErr_Format(PyExc_ValueError, "InvestorID too long: length=%zd (max allowed is %zd)", pQryMMOptionInstrCommRateField_InvestorID_len, (Py_ssize_t)sizeof(self->data.InvestorID));
			return -1;
		}
		strncpy(self->data.InvestorID, pQryMMOptionInstrCommRateField_InvestorID, sizeof(self->data.InvestorID) );
		pQryMMOptionInstrCommRateField_InvestorID = NULL;
	}

	//TThostFtdcOldInstrumentIDType char[31]
	if(pQryMMOptionInstrCommRateField_reserve1 != NULL) {
		if(pQryMMOptionInstrCommRateField_reserve1_len > (Py_ssize_t)sizeof(self->data.reserve1)) {
			PyErr_Format(PyExc_ValueError, "reserve1 too long: length=%zd (max allowed is %zd)", pQryMMOptionInstrCommRateField_reserve1_len, (Py_ssize_t)sizeof(self->data.reserve1));
			return -1;
		}
		strncpy(self->data.reserve1, pQryMMOptionInstrCommRateField_reserve1, sizeof(self->data.reserve1) );
		pQryMMOptionInstrCommRateField_reserve1 = NULL;
	}

	//TThostFtdcInstrumentIDType char[81]
	if(pQryMMOptionInstrCommRateField_InstrumentID != NULL) {
		if(pQryMMOptionInstrCommRateField_InstrumentID_len > (Py_ssize_t)sizeof(self->data.InstrumentID)) {
			PyErr_Format(PyExc_ValueError, "InstrumentID too long: length=%zd (max allowed is %zd)", pQryMMOptionInstrCommRateField_InstrumentID_len, (Py_ssize_t)sizeof(self->data.InstrumentID));
			return -1;
		}
		strncpy(self->data.InstrumentID, pQryMMOptionInstrCommRateField_InstrumentID, sizeof(self->data.InstrumentID) );
		pQryMMOptionInstrCommRateField_InstrumentID = NULL;
	}
