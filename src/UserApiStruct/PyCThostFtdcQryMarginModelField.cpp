...
static int PyCThostFtdcQryMarginModelField_init(PyCThostFtdcQryMarginModelField *self, PyObject *args, PyObject *kwds) {
	...
	const char *pQryMarginModelField_BrokerID = NULL;
	Py_ssize_t pQryMarginModelField_BrokerID_len = 0;

	//TThostFtdcInvestorIDType char[13]
	const char *pQryMarginModelField_MarginModelID = NULL;
	Py_ssize_t pQryMarginModelField_MarginModelID_len = 0;
    ...
    if (!PyArg_ParseTupleAndKeywords(args, kwds, "|s#s#", (char **)kwlist
		...
		, &pQryMarginModelField_BrokerID, &pQryMarginModelField_BrokerID_len
		, &pQryMarginModelField_MarginModelID, &pQryMarginModelField_MarginModelID_len
	...
	//TThostFtdcBrokerIDType char[11]
	if(pQryMarginModelField_BrokerID != NULL) {
		if(pQryMarginModelField_BrokerID_len > (Py_ssize_t)sizeof(self->data.BrokerID)) {
			PyErr_Format(PyExc_ValueError, "BrokerID too long: length=%zd (max allowed is %zd)", pQryMarginModelField_BrokerID_len, (Py_ssize_t)sizeof(self->data.BrokerID));
			return -1;
		}
		strncpy(self->data.BrokerID, pQryMarginModelField_BrokerID, sizeof(self->data.BrokerID) );
		pQryMarginModelField_BrokerID = NULL;
	}

	//TThostFtdcInvestorIDType char[13]
	if(pQryMarginModelField_MarginModelID != NULL) {
		if(pQryMarginModelField_MarginModelID_len > (Py_ssize_t)sizeof(self->data.MarginModelID)) {
			PyErr_Format(PyExc_ValueError, "MarginModelID too long: length=%zd (max allowed is %zd)", pQryMarginModelField_MarginModelID_len, (Py_ssize_t)sizeof(self->data.MarginModelID));
			return -1;
		}
		strncpy(self->data.MarginModelID, pQryMarginModelField_MarginModelID, sizeof(self->data.MarginModelID) );
		pQryMarginModelField_MarginModelID = NULL;
	}
