...
static int PyCThostFtdcQryOrderActionField_init(PyCThostFtdcQryOrderActionField *self, PyObject *args, PyObject *kwds) {
	...
	const char *pQryOrderActionField_BrokerID = NULL;
	Py_ssize_t pQryOrderActionField_BrokerID_len = 0;

	//TThostFtdcInvestorIDType char[13]
	const char *pQryOrderActionField_InvestorID = NULL;
	Py_ssize_t pQryOrderActionField_InvestorID_len = 0;

	//TThostFtdcExchangeIDType char[9]
	const char *pQryOrderActionField_ExchangeID = NULL;
	Py_ssize_t pQryOrderActionField_ExchangeID_len = 0;
    ...
    if (!PyArg_ParseTupleAndKeywords(args, kwds, "|s#s#s#", (char **)kwlist
		...
		, &pQryOrderActionField_BrokerID, &pQryOrderActionField_BrokerID_len
		, &pQryOrderActionField_InvestorID, &pQryOrderActionField_InvestorID_len
		, &pQryOrderActionField_ExchangeID, &pQryOrderActionField_ExchangeID_len
	...
	//TThostFtdcBrokerIDType char[11]
	if(pQryOrderActionField_BrokerID != NULL) {
		if(pQryOrderActionField_BrokerID_len > (Py_ssize_t)sizeof(self->data.BrokerID)) {
			PyErr_Format(PyExc_ValueError, "BrokerID too long: length=%zd (max allowed is %zd)", pQryOrderActionField_BrokerID_len, (Py_ssize_t)sizeof(self->data.BrokerID));
			return -1;
		}
		strncpy(self->data.BrokerID, pQryOrderActionField_BrokerID, sizeof(self->data.BrokerID) );
		pQryOrderActionField_BrokerID = NULL;
	}

	//TThostFtdcInvestorIDType char[13]
	if(pQryOrderActionField_InvestorID != NULL) {
		if(pQryOrderActionField_InvestorID_len > (Py_ssize_t)sizeof(self->data.InvestorID)) {
			PyErr_Format(PyExc_ValueError, "InvestorID too long: length=%zd (max allowed is %zd)", pQryOrderActionField_InvestorID_len, (Py_ssize_t)sizeof(self->data.InvestorID));
			return -1;
		}
		strncpy(self->data.InvestorID, pQryOrderActionField_InvestorID, sizeof(self->data.InvestorID) );
		pQryOrderActionField_InvestorID = NULL;
	}

	//TThostFtdcExchangeIDType char[9]
	if(pQryOrderActionField_ExchangeID != NULL) {
		if(pQryOrderActionField_ExchangeID_len > (Py_ssize_t)sizeof(self->data.ExchangeID)) {
			PyErr_Format(PyExc_ValueError, "ExchangeID too long: length=%zd (max allowed is %zd)", pQryOrderActionField_ExchangeID_len, (Py_ssize_t)sizeof(self->data.ExchangeID));
			return -1;
		}
		strncpy(self->data.ExchangeID, pQryOrderActionField_ExchangeID, sizeof(self->data.ExchangeID) );
		pQryOrderActionField_ExchangeID = NULL;
	}
