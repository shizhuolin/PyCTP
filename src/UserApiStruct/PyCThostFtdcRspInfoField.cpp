...
static int PyCThostFtdcRspInfoField_init(PyCThostFtdcRspInfoField *self, PyObject *args, PyObject *kwds) {
	...
	const char *pRspInfoField_ErrorMsg = NULL;
	Py_ssize_t pRspInfoField_ErrorMsg_len = 0;
    ...
    if (!PyArg_ParseTupleAndKeywords(args, kwds, "|is#", (char **)kwlist
		...
		, &pRspInfoField_ErrorMsg, &pRspInfoField_ErrorMsg_len
	...
	//TThostFtdcErrorIDType int
	self->data.ErrorID = pRspInfoField_ErrorID;

	//TThostFtdcErrorMsgType char[81]
	if(pRspInfoField_ErrorMsg != NULL) {
		if(pRspInfoField_ErrorMsg_len > (Py_ssize_t)sizeof(self->data.ErrorMsg)) {
			PyErr_Format(PyExc_ValueError, "ErrorMsg too long: length=%zd (max allowed is %zd)", pRspInfoField_ErrorMsg_len, (Py_ssize_t)sizeof(self->data.ErrorMsg));
			return -1;
		}
		strncpy(self->data.ErrorMsg, pRspInfoField_ErrorMsg, sizeof(self->data.ErrorMsg) );
		pRspInfoField_ErrorMsg = NULL;
	}
