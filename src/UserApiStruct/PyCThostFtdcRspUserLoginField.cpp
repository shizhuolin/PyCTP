...
static int PyCThostFtdcRspUserLoginField_init(PyCThostFtdcRspUserLoginField *self, PyObject *args, PyObject *kwds) {
    ...
    const char *RspUserLoginField_LastLoginTime = NULL;
    Py_ssize_t RspUserLoginField_LastLoginTime_len = 0;
            
    ///预留信息
    // TThostFtdcReserveInfoType char[65]
    const char *RspUserLoginField_ReserveInfo = NULL;
    Py_ssize_t RspUserLoginField_ReserveInfo_len = 0;
    ...
    if (!PyArg_ParseTupleAndKeywords(args, kwds, "|s#s#s#s#s#iis#s#s#s#s#s#s#s#iis#s#", (char **)kwlist
        ...
        , &RspUserLoginField_LastLoginTime, &RspUserLoginField_LastLoginTime_len 
        , &RspUserLoginField_ReserveInfo, &RspUserLoginField_ReserveInfo_len 
    ...
    ///上次登陆时间
    // TThostFtdcDateTimeType char[17]
    if( RspUserLoginField_LastLoginTime != NULL ) {
        if(RspUserLoginField_LastLoginTime_len >= (Py_ssize_t)sizeof(self->data.LastLoginTime)) {
            PyErr_Format(PyExc_ValueError, "LastLoginTime too long: length=%zd (max allowed is %zd)", RspUserLoginField_LastLoginTime_len, (Py_ssize_t)sizeof(self->data.LastLoginTime));
            return -1;
        }
        // memset(self->data.LastLoginTime, 0, sizeof(self->data.LastLoginTime));
        // memcpy(self->data.LastLoginTime, RspUserLoginField_LastLoginTime, RspUserLoginField_LastLoginTime_len);        
        strncpy(self->data.LastLoginTime, RspUserLoginField_LastLoginTime, sizeof(self->data.LastLoginTime) );
        RspUserLoginField_LastLoginTime = NULL;
    }
            
    ///预留信息
    // TThostFtdcReserveInfoType char[65]
    if( RspUserLoginField_ReserveInfo != NULL ) {
        if(RspUserLoginField_ReserveInfo_len >= (Py_ssize_t)sizeof(self->data.ReserveInfo)) {
            PyErr_Format(PyExc_ValueError, "ReserveInfo too long: length=%zd (max allowed is %zd)", RspUserLoginField_ReserveInfo_len, (Py_ssize_t)sizeof(self->data.ReserveInfo));
            return -1;
        }
        // memset(self->data.ReserveInfo, 0, sizeof(self->data.ReserveInfo));
        // memcpy(self->data.ReserveInfo, RspUserLoginField_ReserveInfo, RspUserLoginField_ReserveInfo_len);        
        strncpy(self->data.ReserveInfo, RspUserLoginField_ReserveInfo, sizeof(self->data.ReserveInfo) );
        RspUserLoginField_ReserveInfo = NULL;
    }
