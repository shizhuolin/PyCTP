...
static int PyCThostFtdcUserSystemInfoField_init(PyCThostFtdcUserSystemInfoField *self, PyObject *args, PyObject *kwds) {
    ...
    const char *UserSystemInfoField_MAC = NULL;
    Py_ssize_t UserSystemInfoField_MAC_len = 0;
            

#if PY_MAJOR_VERSION >= 3
    if (!PyArg_ParseTupleAndKeywords(args, kwds, "|y#y#iy#y#iy#y#y#y#y#", (char **)kwlist
#else
    if (!PyArg_ParseTupleAndKeywords(args, kwds, "|s#s#is#s#is#s#s#s#s#", (char **)kwlist
#endif
        ...
        , &UserSystemInfoField_MAC, &UserSystemInfoField_MAC_len 
    ...
    if( UserSystemInfoField_MAC != NULL ) {
        ...
        // memset(self->data.MAC, 0, sizeof(self->data.MAC));
        // memcpy(self->data.MAC, UserSystemInfoField_MAC, UserSystemInfoField_MAC_len);        
        strncpy(self->data.MAC, UserSystemInfoField_MAC, sizeof(self->data.MAC) );
        UserSystemInfoField_MAC = NULL;
    }
...
#if PY_MAJOR_VERSION >= 3
	PyObject *obj = Py_BuildValue("{s:y,s:y,s:i,s:y,s:y,s:i,s:y,s:y,s:y,s:y,s:y}"
#else
    PyObject *obj = Py_BuildValue("{s:s,s:s,s:i,s:s,s:s,s:i,s:s,s:s,s:s,s:s,s:s}"
#endif
        ...
        ,"MAC", self->data.MAC//, (Py_ssize_t)sizeof(self->data.MAC) 
...
///客户终端的MAC等标识
// TThostFtdcDeviceTagType char[41]
static PyObject *PyCThostFtdcUserSystemInfoField_get_MAC(PyCThostFtdcUserSystemInfoField *self, void *closure) {
    //return PyBytes_FromStringAndSize(self->data.MAC, (Py_ssize_t)sizeof(self->data.MAC));
    return PyBytes_FromString(self->data.MAC);
}

///客户终端的MAC等标识
// TThostFtdcDeviceTagType char[41]
static int PyCThostFtdcUserSystemInfoField_set_MAC(PyCThostFtdcUserSystemInfoField *self, PyObject* val, void *closure) {
    if (!PyBytes_Check(val)) {
    ...
    // memset(self->data.MAC, 0, sizeof(self->data.MAC));
    // memcpy(self->data.MAC, buf, len);
    strncpy(self->data.MAC, buf, sizeof(self->data.MAC));
    return 0;
}
...
static PyGetSetDef PyCThostFtdcUserSystemInfoField_getset[] = {
    ...
    ///客户终端的MAC等标识 
    {(char *)"MAC", (getter)PyCThostFtdcUserSystemInfoField_get_MAC, (setter)PyCThostFtdcUserSystemInfoField_set_MAC, (char *)"MAC", NULL},
